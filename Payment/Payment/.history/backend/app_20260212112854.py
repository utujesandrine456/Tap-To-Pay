...
# --- DATA STORAGE ---
card_balances = {}
transaction_history = [] # Stores last 10-20 transactions
...
@app.route('/')
def index():
    # This MUST match the filename in your /templates folder
...
def topup():
    ...
    if uid and uid in card_balances:
        ...
        print(f"[TOPUP] {uid} | +{amount} | Total: {new_total}")
        return jsonify({"status": "success", "new_balance": new_total}), 200
    
    return jsonify({"error": "No card scanned"}), 400

if __name__ == '__main__':
    # Using debug=True will restart the server whenever you save the file
